...
- A purchase plan has a stable plan_id distinct from ingredient cycles. plan_version increases within that identity; a stored revision also has a unique internal ID. A contingency revises the remaining horizon; a new normal horizon receives a new plan identity.
...
- Each ingredient has interval_days and starting_date. Cycles recur from that anchor and can be OPEN, ORDERED, or SKIPPED; late entry does not shift future dates. No weekday calendars, category inheritance, or reorder-point rules.
